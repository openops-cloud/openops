...
const validateSupportedRelease = (
  minRelease: string | undefined,
  maxRelease: string | undefined,
) => {
  if (minRelease !== undefined && !semver.valid(minRelease)) {
    throw Error(
      `[validateSupportedRelease] "minimumSupportedRelease" should be a valid semver version`,
    );
  }

  if (maxRelease !== undefined && !semver.valid(maxRelease)) {
    throw Error(
      `[validateSupportedRelease] "maximumSupportedRelease" should be a valid semver version`,
    );
  }

  if (
    minRelease !== undefined &&
    maxRelease !== undefined &&
    semver.gt(minRelease, maxRelease)
  ) {
    throw Error(
...
const validatePremiumBlock = (block: BlockMetadata): void => {
  ...
  if (
    block.directoryPath?.includes('/ee/') &&
    !categories.includes(BlockCategory.PREMIUM)
  ) {
    throw Error(
      `[validatePremiumBlock] Premium blocks must be in the 'premium' category`,
    );
  }
  if (
    categories.includes(BlockCategory.PREMIUM) &&
    (!block.minimumSupportedRelease ||
      semver.lt(block.minimumSupportedRelease, '0.27.1'))
  ) {
    throw Error(
      `[validatePremiumBlock] Premium blocks must have a minimum supported release of 0.27.1 or higher`,
    );
  }
};
...
export async function findBlockDirectoryInSource(
  blockName: string,
): Promise<string | null> {
...
export async function findAllBlocks(): Promise<BlockMetadata[]> {
  ...
  return blocks
    .filter((p): p is BlockMetadata => p !== null)
    .sort(byDisplayNameIgnoreCase);
}
