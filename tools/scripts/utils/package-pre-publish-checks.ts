...
const getLatestPublishedVersion = async (
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const response = await axios<{ version: string }>(
        `https://registry.npmjs.org/${packageName}/latest`,
      );
      const version = response.data.version;
      console.info(
        `[getLatestPublishedVersion] packageName=${packageName}, latestVersion=${version}`,
      );
      return version;
    } catch (e: unknown) {
      ...
      if (e instanceof AxiosError && e.response?.status === 404) {
        console.info(
          `[getLatestPublishedVersion] packageName=${packageName}, latestVersion=null`,
        );
        return null;
      }

      console.warn(
        `[getLatestPublishedVersion] packageName=${packageName}, attempt=${attempt}, error=${e}`,
      );
...
const packageChangedFromMainBranch = async (path: string): Promise<boolean> => {
  const cleaned = path.includes('/packages')
    ? `packages/` + path.split('packages/')[1]
    : path;
  if (!cleaned.startsWith('packages/')) {
    throw new Error(
      `[packageChangedFromMainBranch] path=${cleaned} is not a valid package path`,
    );
  }
 ...
 */
export const packagePrePublishChecks = async (
  path: string,
): Promise<boolean> => {
  ...
  const { name: packageName, version: currentVersion } = await readPackageJson(
    path,
  );
  const latestPublishedVersion = await getLatestPublishedVersion(packageName);
  const currentVersionAlreadyPublished =
  ...
  if (currentVersionAlreadyPublished) {
    ...
    if (packageChanged) {
      throw new Error(
        `[packagePrePublishValidation] package version not incremented, path=${path}, version=${currentVersion}`,
      );
    }

    console.info(
      `[packagePrePublishValidation] package already published, path=${path}, version=${currentVersion}`,
    );
    return true;
  }
