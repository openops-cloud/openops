...
describe('tokensService', () => {
  ...
  describe('rotateRefreshToken', () => {
    ...
    it('revokes the entire family when a rotated token is presented again', async () => {
      ...
      // The legitimate client's current token is dead too: the whole chain is
      // untrusted once a replay is observed.
      await expect(
        tokensService.rotateRefreshToken({
    ...
    it('reports a revoked connection as revoked, not as a replay', async () => {
      const original = await issueInitialTokens();
      // Revoking a grant also revokes its tokens, so the claim fails for a
    ...
    it('rejects rotation by a different client without destroying the token', async () => {
      ...
      // A rejected request must leave the credential usable, or the real client's
      // next attempt would look like a replay and kill the whole connection.
