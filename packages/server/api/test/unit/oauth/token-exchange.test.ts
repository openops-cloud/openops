...
jest.mock('../../../src/app/oauth/clients.service', () => ({
...
jest.mock('../../../src/app/oauth/tokens.service', () => ({
  tokensService: {
    mintExchangedApiToken: jest.fn(),
  },
}));

jest.mock('../../../src/app/oauth/signing-key.service', () => ({
  signingKeyService: {
...
import {
  exchangeToken,
  ExchangeTokenParams,
} from '../../../src/app/oauth/token-exchange';
...
describe('exchangeToken', () => {
  ...
  it('inherits the project from the subject token, not from the grant', async () => {
    // A subject token minted for project-2 must not be widened to the grant's
    // project: the two tokens always refer to the same project.
    verifyAccessTokenMock.mockResolvedValue({
      sub: 'user-1',
      aud: MCP_URI,
      client_id: 'client-1',
