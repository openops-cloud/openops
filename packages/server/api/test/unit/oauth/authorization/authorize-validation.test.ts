...
} from '../../../../src/app/oauth/authorization/authorize-validation';
import { oauthConfig } from '../../../../src/app/oauth/config/oauth-config';
import { OAuthClient } from '../../../../src/app/oauth/storage/oauth-model';
...
describe('validateAuthorizeRequest', () => {
  ...
  describe('refuses to redirect when the destination cannot be trusted', () => {
    // The open-redirect boundary: a caller must never turn a `render_error` into a
    // redirect.
  ...
  describe('non-string parameters', () => {
    // `qs` turns `scope[x]=1` into an object; treating that as a string would surface as
    // a 500 rather than an OAuth error.
    it.each([
