...
import {
  invalidGrant,
  serverError,
} from '../../../../src/app/oauth/common/oauth-errors';
...
describe('extractPrincipal with OAuth tokens', () => {
  beforeEach(() => {
    ...
    // Stands in for the real key store, enforcing the audience as production does.
    jest
      ...
      .mockImplementation(async (token, expectedAudience) => {
        ...
        try {
          ...
          // An OAuthError, as production reports: the caller distinguishes those from
          // server faults.
  ...
  /**
   * SERVICE, never USER. Routes restricted to `PrincipalType.USER` act on the session
   * rather than within a project — enterprise's `/switch-project` mints a token for a
   * different one and is exempt from the guard on naming another project. The principal
   * type is therefore the only thing keeping a connection inside the project its token
  ...
  it('acts on the project named by the token, not the one on the grant', async () => {
    // The grant records what was authorized; the token decides what this credential does.
    (grantsService.getActiveGrantOrThrow as jest.Mock).mockResolvedValue({
      ...activeGrant,
      projectId: 'project-1',
    });
  ...
  describe('server faults are not reported as bad credentials', () => {
    // A 401 has clients discard their refresh token and re-authorize, so a database blip
