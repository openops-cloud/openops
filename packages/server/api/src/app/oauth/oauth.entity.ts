...
export const OAuthSigningKeyEntity = new EntitySchema<OAuthSigningKey>({
  ...
  // Partial unique index, mirroring the migration: it is what makes concurrently
  // booting replicas converge on one active key instead of each inserting one.
  indices: [
    {
...
export const OAuthGrantEntity = new EntitySchema<OAuthGrant>({
  ...
  // Deliberately not unique on (clientId, userId): a user may connect the same
  // agent more than once, and each connection is revoked on its own.
  indices: [
    {
