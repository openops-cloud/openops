...
import { invalidGrant, serverError } from './oauth-errors';
import { OAuthSigningKey } from './oauth-model';
import { OAuthSigningKeyEntity } from './oauth.entity';
...
type LoadedKeys = {
  ...
  /** Every key a token may legitimately have been signed with: active + retiring. */
...
async function loadKeys(): Promise<LoadedKeys> {
  ...
  try {
    ...
    // Keys change only on rotation, so a stale copy is still correct. Serving it
    // through a database outage keeps every already-issued token verifiable,
    // where failing would tell every connected agent its credential is invalid.
    if (!cachedKeys) {
      throw error;
    }
...
export const signingKeyService = {
  /**
   * Generates the OAuth signing keypair on first boot so a self-hosted install
   * needs no key configuration. Concurrent replicas race on the partial unique
   * index over `status = 'active'`; the loser simply reuses the winner's key.
   */
  ...
  /**
   * `project_id` is part of the signed payload rather than looked up per request,
   * so a token's authority is fixed for its whole life: it can only ever act on
   * the project it was minted for.
   */
  ...
  /**
   * Verifies an OAuth-issued token, requiring the exact audience the caller
   * expects. Audience is checked here rather than by callers so no code path can
   * accept a token minted for a different resource.
   */
