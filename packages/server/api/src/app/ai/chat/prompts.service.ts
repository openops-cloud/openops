...
export const getMcpSystemPrompt = async (): Promise<string> => {
  const baseMcpPrompt = await loadPrompt('mcp/mcp.txt');
  ...
  const tablesPrompt = await loadPrompt('mcp/mcp-tables.txt');
  const analyticsPrompt = await loadPrompt('mcp/mcp-analytics.txt');
