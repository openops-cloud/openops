...
export async function addFields(
  ...
  logger.debug(
    `[Seeding Tag-Owner mapping table] Added field Owner tag value with id: ${primaryField.id}`,
  );
...
async function addField(
  ...
  logger.debug(
    `[Seeding Tag-Owner mapping table] Added field ${fieldBody.name} with id: ${field.id}`,
  );
