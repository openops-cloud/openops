export const RESIZABLE_PANEL_GROUP = 'ResizablePanelGroup';
export const RESIZABLE_PANEL_IDS = {
  MAIN: 'main-container',
  RIGHT_SIDEBAR: 'right-sidebar-container',
  LEFT_SIDEBAR: 'left-sidebar-container',
  BUILDER_LEFT_SIDEBAR: 'builder-left-sidebar-container',
  BUILDER_RIGHT_SIDEBAR: 'builder-right-sidebar-container',
  BUILDER_MAIN: 'builder-main-container',
  SECONDARY_LEFT_SIDEBAR:
    'secondary-left-sidebar-panelcondary-left-sidebar-panel-container',
};
