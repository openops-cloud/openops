...
>(
  (
    ...
    const renderItems = remainingBlocksCount
        ...
        >
          ...
          {remainingBlocksCount ? (
            <span className="text-muted-foreground text-xs leading-4 py-.5">
              {t('+{remainingBlocksCount} more', {
                remainingBlocksCount: remainingBlocksCount,
              })}
            </span>
          ) : null}
        </div>
...
>((props, ref) => {
  const { onSearch, open } = useMultiSelect();
...
>(({ className, ...props }, ref) => {
  return (
    <ScrollArea className="h-full" viewPortClassName={'max-h-[200px]'}>
      <CommandList
        ref={ref}
        className={cn('py-1 px-0 max-h-full', className)}
        {...props}
      />
    </ScrollArea>
  );
