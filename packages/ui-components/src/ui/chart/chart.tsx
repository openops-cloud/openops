...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        'border-border/50 bg-background gap-1.5 text-foreground rounded-lg border px-2.5 py-1.5 text-xs shadow-xl grid min-w-32 items-start',
        className,
      )}
    >
      ...
      <div className="grid gap-1.5">
        {payload
          ...
          .map((item, index) => {
            ...
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
            const indicatorColor = color || `var(--color-${key})`;
              ...
              >
                {formatter && item?.value !== undefined && item.name ? (
                  ...
                  <>
                    ...
                    >
                      <div className="grid gap-1.5 mr-1">
                        {nestLabel ? tooltipLabel : null}
...
type PlotAreaBackgroundProps = {
  offset?: { left?: number; top?: number; width?: number; height?: number };
  topSpacing?: number;
};

function PlotAreaBackground({
  offset = {},
  topSpacing = 8,
}: PlotAreaBackgroundProps): React.JSX.Element {
  ...
  return (
    <rect
      ...
      y={top - topSpacing}
      width={width}
      height={height + topSpacing}
      fill="hsl(var(--background-800))"
    />
  );
}
