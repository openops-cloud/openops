...
describe('googleVertexProvider', () => {
  ...
  describe('createLanguageModel', () => {
    ...
    test('should work without provider settings', () => {
      const params = {
        apiKey: MOCK_SERVICE_ACCOUNT_JSON,
        model: 'gemini-2.5-pro',
      };
    ...
    test('should work with empty provider settings', () => {
      const params = {
        apiKey: MOCK_SERVICE_ACCOUNT_JSON,
        model: 'gemini-2.5-pro',
        providerSettings: {},
    ...
    test('should work with partial provider settings', () => {
      const params = {
        apiKey: MOCK_SERVICE_ACCOUNT_JSON,
        model: 'gemini-2.5-pro',
        providerSettings: {
    ...
    test('should work without credentials when apiKey is empty', () => {
      const params = {
        apiKey: '',
        model: 'gemini-2.5-pro',
        providerSettings: {
    ...
    test('should work without credentials when apiKey is only whitespace', () => {
      const params = {
        apiKey: '   ',
        model: 'gemini-2.5-pro',
        providerSettings: {
    ...
    test('should call provider function with correct model name', () => {
      const mockProvider = jest.fn().mockReturnValue(mockLanguageModel);
      createVertexMock.mockReturnValue(mockProvider);
    ...
    test('should handle complex service account JSON structure', () => {
      const complexServiceAccount = {
        ...MOCK_SERVICE_ACCOUNT,
        universe_domain: 'googleapis.com',
        additional_field: 'test-value',
      };
      const complexJson = JSON.stringify(complexServiceAccount);
      ...
      expect(createVertexMock).toHaveBeenCalledWith({
        location: 'us-central1',
        project: 'test-project',
        googleAuthOptions: { credentials: complexServiceAccount },
      });
    });
  });
});
