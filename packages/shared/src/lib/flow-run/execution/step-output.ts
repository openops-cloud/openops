...
export type StepOutput =
  ...
  | GenericStepOutput<ActionType.SPLIT, SplitStepResult>
  | GenericStepOutput<
      | Exclude<
          ActionType,
          ActionType.LOOP_ON_ITEMS | ActionType.BRANCH | ActionType.SPLIT
        >
      | TriggerType,
      unknown
    >;
...
type SplitStepResult = {
  evaluatedOptions: { optionName: string; conditionsMatched: boolean }[];
  selectedOptionName: string | undefined;
};

export class SplitStepOutput extends GenericStepOutput<
  ActionType.SPLIT,
  SplitStepResult
> {
  static init({ input }: { input: unknown }): SplitStepOutput {
    return new SplitStepOutput({
